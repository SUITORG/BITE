...
function doPost(e) {
    ...
    try {
        ...
        if(action === "askGemini") {
            ...
            // Modelos EXTRAñDOS de tu lista de diagnóstico real
...
function sendAdminAlert(lead, config) {
  ...
  const message = `ð¨ NUEVO LEAD (${companyName}): ${lead.nombre}\n\nTel: ${lead.telefono}\nEmail: ${lead.email}`;

  if (adminEmail && adminEmail.includes("@")) { MailApp.sendEmail({ to: adminEmail, subject: `ð¨ NUEVO LEAD: ${lead.nombre}`, body: message }); }
