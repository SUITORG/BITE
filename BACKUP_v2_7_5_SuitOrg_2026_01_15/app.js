...
const app = {
    ...
    agents: {
        run: (agentName) => {
            ...
            title.innerText = `ð¤ ${agentName} pensando...`;
            output.value = "Conectando con la red neuronal de EVASOL...";

            setTimeout(() => {
                title.innerText = `â Respuesta del ${agentName}`;
                let response = "";
    ...
    auth: {
        login: (userOrEmail, password) => {
            ...
            if (user.rol !== 'DIOS') {
                ...
                if (currentCredits <= 5) {
                    alert("â ï¸ ADVERTENCIA: Te quedan solo " + currentCredits + " créditos. Por favor, comunícate con el equipo de soporte técnico para una recarga.");
                }
            }
    ...
    maintenance: {
        resetCompany: () => {
            if (!confirm("â ï¸ ¿ESTñS SEGURO? Esto borrará leads y proyectos.")) return;
            // Mock action
            app.data.Leads = [];
            app.ui.renderLeads();
    ...
    ui: {
        ...
        renderPillars: (company) => {
            ...
            const pillars = [
                { title: 'MISIñN', text: company.mision, icon: 'fa-bullseye' },
                { title: 'VISIñN', text: company.vision, icon: 'fa-eye' },
                        ...
                        ${logs.length ? logs.sort((a, b) => new Date(b.fecha_hora) - new Date(a.fecha_hora)).map(l => `
                            <div style="padding:8px; border-bottom:1px solid #eee; border-left: 3px solid ${l.tipo_evento === 'AUTO' ? '#2196F3' : '#4CAF50'}; margin-bottom:5px; background: rgba(0,0,0,0.02)">
                                <div class="flex-between">
                                    <span class="text-small"><strong>${l.tipo_evento === 'AUTO' ? 'ð¤ SISTEMA' : 'ð¤ ' + l.usuario}</strong></span>
        ...
        syncKnowledge: async () => {
            ...
            try {
                ...
                const data = await res.json();
                if (data.success) {
                    alert("â Sincronización exitosa");
                    await app.loadData();
        ...
        bindEvents: () => {
            ...
            document.getElementById('public-lead-form').onsubmit = async (e) => {
                ...
                try {
                    ...
                    console.error("Fetch Error:", err);
                    alert("â Error al enviar datos. Inténtalo más tarde.");
                } finally {
                    btn.innerText = originalText;
                    btn.disabled = false;
                    form.dataset.submitting = "false";
        ...
        sendMessage: async () => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    app.agents.addMessageToUI('ai', "â Error: " + fullError);
                }
            } catch (e) {
                console.error(e);
                app.ui.updateConsole("AI_CONN_FAIL", true);
                app.agents.addMessageToUI('ai', "â Error de conexión con el servidor.");
            } finally {
